...
if ($data['transaction_status'] == "settlement" OR $data['transaction_status'] == "capture" OR $data['transaction_status'] == "accept") {
    // $query = mysqli_query($conn, "UPDATE ba_transaksi_ebook SET status_transaksi= '7', tanggal_dibayar = NOW(), payment_type='".$data['payment_type']."' WHERE invoice = '$order_id'");
    ...
    $stmt[] = $conn->query("UPDATE ba_transaksi_ebook SET status_transaksi= '7', tanggal_dibayar = NOW(), tgl_aktif = DATE_ADD(NOW(), INTERVAL '$lama' DAY), payment_type='".$data['payment_type']."' WHERE invoice = '$order_id'");
    if (in_array(false, $stmt) OR in_array(0, $stmt)) {
        ...
        $conn->commit();
        $respon['pesan'] = "Yeyee berhasil edit transasi dengan kode invoice ".$order_id;
        die(json_encode($respon));
    }
