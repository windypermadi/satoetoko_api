...
if ($data['transaction_status'] == "settlement" or $data['transaction_status'] == "capture" or $data['transaction_status'] == "accept") {
    ...
    $transaksidetail =  mysqli_query($conn, "SELECT b.id_transaksi, a.id_transaksi_detail, a.status_pembelian, d.lama_sewa, c.id_supplier 
    ...
    $temp = $conn->query("SELECT * FROM saldo WHERE id_supplier = ''")->fetch_assoc();

    $conn->begin_transaction();
    $sql    = mysqli_query($conn, "SELECT a.id_transaksi_detail, a.status_pembelian, d.lama_sewa, a.harga_normal, a.harga_diskon
    FROM ebook_transaksi_detail a 
    JOIN ebook_transaksi b ON a.id_transaksi = b.id_transaksi
    JOIN master_item c ON a.id_master = c.id_master
    JOIN master_ebook_detail d ON c.id_master = d.id_master
